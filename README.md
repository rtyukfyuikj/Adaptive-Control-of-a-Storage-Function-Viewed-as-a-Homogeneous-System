...
を用いる($V$ は仮定1が保証する次数 $\mu>2r_{\max}$ の同次Lyapunov関数, $\alpha,\beta>0$ は設計パラメータ, $\|\cdot\|_r$ は同次ノルム)。同次関数の性質 (Young/Hölderの不等式)を用いて $\alpha+\beta\tau$ を十分小さく選ぶと, ある $h>0$ が存在して
...
- $\nu<0$ のとき $1+\nu/\mu<1$ であり, 同じ形の不等式は逆に $\tilde V$ が大きい領域で成立する。これは大きな $\tilde V$(したがって大きな $\|x(t)\|_r$)が有界な集合の外側で減少し, 解がその集合に引き込まれて留まることを意味する(UUB)。原点近傍では不等式が保証されないため, $x(t)\to0$ までは主張できない。

すなわち, 同次次数 $\nu$ の符号は, 非線形減衰項が「原点近傍で線形より強く効くか ($\nu>0$, ASを生む)」, それとも「遠方でのみ効き, 近傍では効かないか ($\nu<0$, UUBに留まる)」を決めている。不等式(5)の詳細な導出 (LKFの上下界評価, 微分の評価, 補題1による相殺)は原論文の定理1証明を参照。本モデルへの適用(仮定1の検証, 定理1/定理2の使い分け)は §1.2でまとめて行う。

## 1.2 貯留関数法の基礎式(引用)

出典: 角屋睦, 永井明博, 「流出解析手法(その10) — 4. 貯留法—貯留関数法による洪水流出解析」, 農業土木学会誌, 46(10), pp.747–753, 1978(証明等は原論文参照)。星・山岡モデルの由来として必要最小限の式のみ引用する。

#### 星・山岡モデルによる分布遅延型拡張(補足)

角屋・永井(1978)の貯留関数法は, 貯留量-流出量関係を単一のべき関数 $S=K\,Q^{P}$ で表すため, 増水期と減水期で $S$-$Q$ 平面上に異なる経路を描く二価性(ループ)を表現できない(古典的な貯留関数法では遅れ時間 $T_l$ の導入によりこの点が回避されてきた)。星・山岡(1982)は非定常項 $K_2\,\mathrm{d}(Q^{P_2})/\mathrm{d}t$ を追加した
...
という構造式を提示し, 近似を用いずに二価性を表現できることを示した (出典: 星清・山岡勲,「雨水流法と貯留関数法の相互関係」, 第26回水理講演会論文集, pp.273–278, 1982; 式(6)は同論文の式(21)(河道ブロックでは式(22))に対応し, ここでの引用は「＜参考資料３＞貯留関数法とその適用法」(kohyo-21-k133-1-3), pp.33–35 による)。$K_1,K_2,P_1,P_2$ は流域(または河道)パラメータ, $S$：貯留量, $Q$：流出量(または直接流出量) である。

式(6)を, 角屋・永井(1978)の水収支式を現在時刻 $t$ 基準・移動窓 $[t-\tau,t]$ の形に書き換えた分布遅延型の水収支
...
(水収支自体を指数付きの一般化流束に書き換えるという追加のモデル化の仮定は不要で, 標準形のまま用いる)と組み合わせると
...
が得られる(近似を含まない厳密な関係)。星・山岡(1982)自身の数値計算法 (前掲書 式(23))にならい, 状態変数を
...
によって定義すると $\mathrm{d}X/\mathrm{d}t = \mathrm{d}(Q^{P_2})/\mathrm{d}t$ であるから, 式(8)は $X$ について直接解け, $Q(t-1)$ の挿入のような離散化の技巧を経由せずに連続時間の状態方程式
...
とおくと $\dot X(t)=F(X(t))+R(t)$(式(10)の項の再分類)。これは§1.1で引用した文献(Aleksandrov, Efimov, Fridman, 2023)の定理2が扱う摂動系 $\dot x=F(x(t))+\int_{t-\tau}^{t}G(x(s))\mathrm{d}s
+\int_{t-\tau}^{t}\mathcal R(s,x(s))\mathrm{d}s$ において $G\equiv0$ とし, 摂動 $\mathcal R$ を $R(t)$ の被積分関数とみなした場合に対応する。同定理は, 摂動 $\mathcal R$ が仮定2「$|\mathcal R_i(t,x)|\le a\|x\|_r^{\,r_i+\varrho}$ ($a>0,\varrho+r_{\min}>0$)」を満たすとき, $\nu>0,\varrho\ge\nu$ でゼロ解 AS, $\nu<0,\varrho\le\nu$ で解UUBとなることを保証する。
...
$R(t)$ の被積分関数のうち $X(s)^{1/P_2}$ の次数は $\varrho=1/P_2-1$ である。降雨 $I(s)$ については, 貯留関数法の水収支式 $S=\int I\,dt-\int Q\,dt$ (角屋・永井 1978, §1.2)と, 洪水イベント全体での流出係数(総流出量/総降雨量)がおおむね1に近いという経験則により, $I(s)$ は $Q(s)=X(s)^{1/P_2}$ と同程度の大きさをもつとみなせる。したがって $R(t)$ の被積分関数全体($I(s)$, $X(s)^{1/P_2}$ とも)は同じ次数 $\varrho=1/P_2-1$ で仮定2を満たす。定理2のAS条件 $\varrho\ge\nu$ ($\nu=P_1/P_2-1$)は

$$
\frac1{P_2}-1 \ge \frac{P_1}{P_2}-1 \iff P_1\le1
$$

<div align="right">式 (13)</div>

であり, 星・村上(1987)の代表値 $P_1=0.6$(式(15))はこれを満たす。式(16)($P_1>P_2$, $\nu>0$)とあわせて, 定理2によりゼロ解ASが結論できる。

$I(s)\le I_{\max}$, $X(s)\in[0,X_{\max}]$ のもとで

$$
|R(t)| \le \frac{\tau}{K_2}\bigl(I_{\max}+X_{\max}^{1/P_2}\bigr)
$$

<div align="right">式 (14)</div>

により一様有界である。主項 $F(X)$ は $P_1,P_2$ の値によらず単独で $r$-同次(次数 $\nu=P_1/P_2-1$)であり, 複数項の指数を揃える必要があった旧稿の制約(「$P_1=1$ でなければ定理1を適用できない」)は生じない。

#### パラメータの制約

「＜参考資料３＞貯留関数法とその適用法」(kohyo-21-k133-1-3, pp.33–35)によれば, 式(6)のパラメータには次の制約・代表値がある。

- $K_1,K_2>0$(貯留量の非負性から要求される正値制約), $P_1,P_2>0$ (木村モデルの $P$ と同様, 観測ハイドログラフへの非線形最適化(ニュートン法など)で同定される経験的な形状パラメータ)。

- 星・村上(1987)による総合化式(前掲書 式(24)): 

$$
K_1 = 2.823\,f_c\,A^{0.24}, \qquad
        K_2 = 0.2835\,K_1^{2}\,\bar r_e^{-0.2648}, \qquad
        P_1 = 0.6, \qquad P_2 = 0.4648
$$

<div align="right">式 (15)</div>

 ここに $f_c := (n/\sqrt{i})^{0.6}$(流域平均粗度), $n$：等価粗度 (s/m$^{1/3}$), $i$：平均斜面勾配(無次元), $A$：流域面積(km$^2$), $\bar r_e$：平均有効降雨強度(mm/h)。

#### $P_1,P_2$ の設定条件

以上を踏まえ, 式(6)のパラメータ $P_1,P_2$ には次の条件が課される。

- **適切性**: $Q(t)\ge0$(流量の非負性, $Q=0$ を含む)のもとで式(6)の $Q^{P_1}$, および変数変換 $X(t):=Q(t)^{P_2}\Leftrightarrow Q(t)=X(t)^{1/P_2}$ が矛盾なく定義されるためには $P_1>0$ かつ $P_2>0$ が必要である。

- **主項の同次次数の符号**: 式(11)の主項の次数は $\nu=P_1/P_2-1$ である。摂動 $R(t)$ を無視した名目系 $\dot X=F(X)$ が定理1(i)の意味でAS favorable($\nu>0$)となる条件は 
...
<div align="right">式 (16)</div>
 ...
 である($P_1=1$ を仮定した場合の条件 $0<P_2<1$ は, この条件の $P_1=1$ の場合に相当する)。

- **文献の代表値との整合**: 星・村上(1987)の代表値 $P_1=0.6,P_2=0.4648$(式(15))は $0.6>0.4648$ より条件(16)を満たす。

## 1.3 MRAC型適応制御の式展開

対応: `appendix_q_model_adaptive_control_simulation.simulate_adaptive_q_model`

$Q(t)$\[m$^3$/s\] を流出量, $I(t)$\[mm\] を降雨強度, $\tau$\[min\] を分布遅延の積分窓長とする(詳細は§6.1, §4参照)。

流出量の状態を, §1.2の式(9)と同じ変換により $X(t):=Q(t)^{P_2}$($Q(t)=X(t)^{1/P_2}$)と表記する。これは本節限りの記号の付け替えではない。星・山岡モデルの状態変数そのものである。

理由は次の通りである。式(9)のもとでは恒等式 $X(s)^{\alpha_2}=X(s)^{1/P_2}=Q(s)$($\alpha_2:=1/P_2$)が成り立つ。この恒等式により, 窓積分項は式(8)どおりの線形な $\int Q(s)\,\mathrm{d}s$ に帰着する。これが指数 $\alpha_2:=1/P_2$ の選択根拠である。もし単に $X(t):=Q(t)$ と置くとこの恒等式は成り立たない。窓積分項は $\int Q(s)^{1/P_2}\mathrm{d}s$ という, 式(8)に現れない項になる。これでは星・山岡モデルの厳密な言い換えにならない。

そこで星・山岡モデル(式(11), §1.2)にならい, 瞬時減衰項の指数を $\alpha_1:=P_1/P_2$, 窓積分項の指数を $\alpha_2:=1/P_2$ とする(いずれも既知・固定)。§6.1の $\alpha:=1/p$ は, $\alpha_1=\alpha_2$ の特別な場合に相当する。

式(53)では瞬時減衰項の係数が暗に $1$ に固定されている。ここではこれを未知パラメータ $\theta_3:=K_3$ として一般化し, MRACによりこの $\theta_3$ のみをオンライン推定する。降雨側・流出側の係数 $\theta_1:=A^{\alpha_2}/K_2$, $\theta_2:=1/K_2$($A$: 流域面積, 既知)は, §1.2の $K_1,K_2$ と同様に*既知の物理定数*として扱う(推定はしない; $\theta_2$ が小さい理由は後述)。真の系は
...
(最右辺は $X(s)^{\alpha_2}=X(s)^{1/P_2}=Q(s)$ の恒等式による書き換えであり, 近似ではない)。式(17)は§1.2の $\dot X=F(X)+R(t)$(式(11))と*同じ形*であり, $F(X):=-\theta_3X^{\alpha_1}$ とおけば, 仮定1・仮定2の検証(補助系 $\dot X=F(X)$ のAS, $R(t)$ の成長次数)は§1.2の議論を $K_1\to\theta_3$ の対応でそのまま流用できる(真の系自身のAS/UUBの根拠; 後述のMVTのステップで用いる $X(t)\in[X_{\min},X_{\max}]$ の有界性はこれによる)。
...
`integrated_rain_to_pump_simulation_test_sinc.py`の実装では, 式(19)の $I(s)$ に生の降雨深(観測周期あたりの積算降雨量) $I(s)$\[mm\]をそのまま用いるのではなく, 流域面積 $A$\[km$^2$\] を用いて体積流量に換算した
...
を用いる($\Delta t_{\min}$\[min\]は観測周期, $c_V$ は降雨深$\times$面積を体積に直す単位換算係数(1mm深さ$\times$<!-- -->1km$^2

$$
=$<!-- -->1000m$^3$; 式(56)の $c_A:=A\cdot1000/60$ とは異なり, 秒への換算を含まない別の定数)である)。単位は $I_{\mathrm{flow}}(t)$\[m$^3$/min\]であり, 状態 $X(t)$ もこれと同じm$^3$/minの体積流量として扱う。これにより式(19), (20)の $I(s), X(s)$ は最初から同一単位系(体積流量)にあり, 面積換算を $\theta_1$ に個別に含める必要がなくなるため, $\theta_1=\theta_2=1/K_2$(本節冒頭の $A^{\alpha_2}$ 因子は不要)となる。

対象流域(`CatchmentModel`, $A=1.5$km$^2$, $\Delta t_{\min}=5$分)と, §5の横浜市雨水管理計画確率年別降雨強度式 (5/10/20/30年確率の4イベント, 中央集中型ハイエトグラフ)を用いた場合, $I_{\mathrm{flow}}(t)$ のおおよそのスケール(非零区間の最小値・最大値)は次の表の通りである($I_{\mathrm{flow}}(t)$, 式(21)の実測値, 非零区間のみ; m$^3$/s換算では概ね1–68m$^3$/s)。
...
式(17)において $\theta_3$ は線形に現れる。そのため, 状態 $X(t)$ 自身から作った候補Lyapunov関数 $V=X^\mu+\tfrac{\gamma_3}{2}
...
とする($R(t)$ は式(17)と*同一の*既知量; $\theta_1,\theta_2$ が既知定数であるため, 真の系にも推定器にも同じ $R(t)$ をそのまま使える)。 $\Phi_Q(t)$(したがって $R(t)$)の計算には series-parallel構成を踏襲し, $\hat X$ 自身の履歴ではなく*真の* $X(t)$ の履歴を用いる。
...
($e(t)$\[m$^3$/s\] は状態誤差, $\tilde\theta_3$($\theta_3$ と同じ次元)はパラメータ誤差)と定義すると, 式(17), (22)を引き算すると, $R(t)$ が真の系・推定器の両方に*同一の値*で現れるため恒等的に相殺し
...
式(31)を式(30)の $[\,\cdot\,]$ に代入する。すなわち, 適応則の右辺を, それが打ち消すべき回帰項*そのもの*に等しくなるよう選んだことを直接確認する:
...
($\gamma_3$ が代入と同時に約分して消えるため, $\tilde\theta_3$ の値によらず恒等的に成り立つ)。式(30)に代入すると
...
のみが残る。$\hat\theta_3$ は $[\theta_{3,\min},\theta_{3,\max}]$ ($\theta_{3,\min}>0$)にprojectionし $\hat X(t)\ge0$ を保つ限り, 式 (28)(単調性)より $e(X^{\alpha_1}-\hat X^{\alpha_1})\ge0$ なので, 式(33)より $\dot V\le0$ である(旧稿のような Taylor近似の正当性に頼らない, 厳密な不等式)。$V\ge0$ かつ単調非増加なので $e(t),\tilde\theta_3(t)$ はすべて有界(Lyapunov安定)。さらにこれらの有界性から式(27)右辺($\dot e$)も有界, すなわち $e(t)$ は一様連続であり, Barbalatの補題より $\dot V(t)\to0$, したがって
...
X_{\max}^{\alpha_1-1})>0$ とおけば $e(X^{\alpha_1}-\hat X^{\alpha_1})\ge c\,e^2$ が成り立つ。したがって式(34)から
...
$R(t)$(したがって $\Phi_I(t),\Phi_Q(t)$)は分布遅延の窓積分を含むが, 式(26)で見た通り真の系・推定器の双方に同一の値で現れ誤差方程式 $\dot e$ から完全に消去されるため, 本節の $V=\tfrac12e^2+\tfrac1{2\gamma_3}\tilde\theta_3^2$ による解析は遅延項の影響を一切受けない。したがって§1.1のような無入力系の同次安定性で必要となるLyapunov–Krasovskii汎関数(式(4)のような追加項)は不要であり, 式(33)–(35)がそのまま完結した証明になっている($\theta_1,\theta_2$ を既知として除いたことで, 3パラメータ版よりもむしろ議論が単純化されている)。
...
とし, タップ $k$ の適応度を, 式(38)と同じ sinc カーネルを用いた *直接照合*
...
で定義する(実装上は, ベクトル $[\bm x(t);\,Q_{\mathrm{norm}}(t)]\in\mathbb R^{M+1}$ に式(38)の $K$ を適用し, 最終行(=$Q_{\mathrm{norm}}(t)$との比較) の先頭 $M$ 成分 $K[0{:}M-1,\,M]$ を取り出すことで $f(t)=(f_0(t),\dots,f_{M-1}(t))$ を得る)。レプリケータ動力学は原型と同じ形
...
とする($\omega(t)$はシンプレックス上を動き, $\sum_k\omega_k(t)=1$ を保つ)。式(46)を時刻 $0$ から観測終了時刻 $T$ まで数値積分 (`scipy.integrate.solve_ivp`, 既定RK23法)し, 終端の重み $\omega(T)$ が最大となるタップ
...
を計算・出力する。この式はユーザー指定の経験的な目安式であり, 理論的な導出は与えられていない。$\tau_{\mathrm{suggest}}$ は移動窓の*長さ*であり正でなければならないため, 式(50)は
...
2.  **流出量のオンライン推定**: 分布遅延をもつ貯留関数型の状態方程式 $\dot X(t)$(式(17), §1.3)に対し, 未知パラメータ $\theta_3$ のみをMRAC型適応則で推定する並列モデル $\hat X(t)$ (式(22))を数値積分する (§6, §1.3, `simulate_adaptive_q_model`)。
...
| $Q(t)$                     | m$^3$/s               | 流出量(真値)                                                                                                                                                                                                                                  |
| $\hat Q(t)$                | m$^3$/s               | 流出量の推定値, $:=\hat X(t)^{1/P_2}$(式(23))                                                                                  |
| $X(t)$                     | (m$^3$/s)$^{P_2}$     | §1.3の状態, $:=Q(t)^{P_2}$(式(9)と同じ変換; 物理量そのものではない) |
| $\hat X(t)$                | (m$^3$/s)$^{P_2}$     | $X(t)$ の並列モデル推定値(式(22)で積分される状態そのもの)                                                                                                       |
...
| $R(t)$                     | (m$^3$/s)$^{P_2}$/min | 既知の外生入力 $\theta_1\Phi_I(t)-\theta_2\Phi_Q(t)$(式(18))                                                                               |
| $\Phi_I(t),\Phi_Q(t)$      | –                     | 回帰信号(降雨側・流出量側の窓積分, $\Phi_Q$ の被積分関数は式 (20)より $X(s)^{\alpha_2}=Q(s)$)                                                    |
| $e(t)$                     | (m$^3$/s)$^{P_2}$     | 状態誤差 $X(t)-\hat X(t)$(物理量Q(t)の誤差ではない, 式(24))                                                                                            |
| $\gamma_3$                 | –                     | 適応ゲイン                                                                                                                                                                                                                                    |
...
式(53)は硬い(stiff)非線形の瞬時減衰項 $-Q^{1/p}$ を含むため, 前進Euler(陽解法)では $\Delta t_{\min}=5$分程度の粗い刻みで振動的な数値不安定 (棘状のオーバーシュート)を起こす。そこで瞬時減衰項のみを後退Euler(陰解法)で扱い, 分布遅延積分項(観測履歴に基づく既知量)は台形則により陽的に評価する:
...
式(53)では瞬時減衰項の係数が暗に $1$ に固定されているが, これを未知パラメータ $\theta_3:=K_3$ として一般化し, 降雨側・流出側の係数 $\theta_1,\theta_2$ は既知の物理定数として扱った上でMRAC型オンライン推定則(真の系・並列モデル, 誤差方程式, Lyapunov関数, 適応則)により $\theta_3$ のみを推定する式展開は§1.3(第1章)にまとめてある。以下の $\Phi_I,\Phi_Q$ 等の記法はそこでの定義(式(17), 指数 $\alpha_2:=1/P_2$)をそのまま用いる。
...
真の系(式(17))・推定器(式(22))はいずれも, 瞬時減衰項のみ後退Euler(`brentq`による求根)+分布遅延積分項は台形則による陽的評価, という半陰的Euler法で数値積分する。適応則(式(31))は前進Eulerで積分する。3つのモード
...
- `"adaptive"`: `"fixed"`と同じ初期値から, 式 (31)で $\gamma_3$ を用いてオンライン推定する。
...
と拡張し, 式(17)(22)の $\Phi_I$ をすべて $\Phi_I^{\mathrm{ext}}$ に置き換えるものである($c_A$は式(56))。$\Phi_I^{\mathrm{ext}}$ は $Q(t),\hat Q(t)$ にも未知パラメータにも依存しない既知の外生信号であるため, §1.3のMRAC設計(交差項の相殺, $\dot V\le0$)は $\Phi_I\to\Phi_I^{\mathrm{ext}}$ の置換のもとでそのまま成立する。数式の詳細な導出・単位整合の議論・$Q_R(t)$取得における代数ループ回避(2パス方式)は `pump_feedback_theta1_adaptive_law.tex` にまとめてある。

**現在の`rain_runoff_delay_replicator.py`のパイプラインでは, この補正は適用されていない**($Q_R=\text{None}$ が既定で渡され, $\Phi_I^{\mathrm{ext}}=\Phi_I$ に一致する, すなわち $\int I\,\mathrm{d}s$ のみによる式 (17)(22)がそのまま使われる)。ポンプ吐出量 $Q_R(t)$ は §8で述べる片方向結合により別途計算されるが, 流出量 $\hat Q(t)$ の計算には反映されず, プロット表示にのみ用いられる。
...
揚程曲線(式(61))とプラント曲線の交点(交点は高々1つ; 実行不可能なら $Q_{\mathrm{op}}$ は不定, 上限 $Q_{\mathrm{ad,max}}$ でクリップ): 
...
<div align="right">式 (76)</div>
 ...
 ON中は $N=N_{\max}$ で式(74), OFF中は $Q_{\mathrm{op}}=0,N=0$。水位はEuler更新(刻み $\Delta t_s$, 既定1秒): 
...
<div align="right">式 (81)</div>
 ...
 一方プラント側の揚程は台数によらず合計流量 $Q_{\mathrm{total}}$ で決まるため, 動作点方程式(73)は次のように一般化される: 
...
<div align="right">式 (83)</div>
 ...
 ($m=1$ で式(73)(74)に厳密に一致し, `verify_multi_matches_single_pump` で数値的にも確認済み)。
...
<div align="right">式 (86)</div>
 ...
 と決める($n_p=1$ で単一ポンプ版のON/OFF判定(式(75) (76))に一致)。水位更新・合計消費電力は式(77) と同型のまま $Q_{\mathrm{op}}\to Q_{\mathrm{total}}$ とし, 
...
§6(§1.3のMRAC型適応推定を用いる場合は, 式 (23)により状態 $\hat X(t)$ から変換した後の, 物理量としての $\hat Q(t)=\hat X(t)^{1/P_2}$)で得た $\hat Q(t)$($\Delta t_{\min}=5$分刻み)を, 線形補間(`resample_to_1s`)により1秒刻みの流入量 $Q_{\mathrm{in}}(t_{\mathrm{sec}})$ に変換し, §7のポンプ場モデルへ毎秒与える (`run_realtime_stepwise` が式 (75)(76)(77) を1秒ごとに逐次実行する)。得られた1秒刻みのポンプ吐出量は, 区間内の水量 (平均流量$\times$区間長)を保つように, $\Delta t_{\min}$ 幅の各区間で区間平均を取ってダウンサンプルし, $Q_R(t)$(5分刻み)を得る:
...
<div align="right">式 (89)</div>

($B_i$ は $i$ 番目の $\Delta t_{\min}$ 区間に属する1秒刻みインデックスの集合)。

現在の実装では, この $Q_R(t)$ は $\hat Q(t)$ の計算(§1.3)には用いられない(片方向結合: $\hat Q(t)\to$ポンプ場$\to Q_R(t)$のみ)。すなわち, 式(60)の水収支補正拡張は組み込まれておらず, $\Phi_I^{\mathrm{ext}}=\Phi_I$ の状態(§6.4)にとどまる。$Q_R(t)$ はポンプ吐出量・累積吐出水量の可視化(`plot_rain_runoff_and_pump`)にのみ用いられる。
...
|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| §5, 式(52)                                                                                                                                | `rrl_storage_function_simulation.generate_synthetic_rainfall`                                                                                                           |
| §6.1, 式(53)–(57)                                    | `appendix_q_model_simulation.simulate_appendix_q_model`                                                                                                                 |
| §1.3, 式(17)–(35)                                          | `appendix_q_model_adaptive_control_simulation.simulate_adaptive_q_model`                                                                                                |
| §6.4, 式(60)                                                                                                                         | 同ファイル `Q_R` 引数; `pump_feedback_theta1_adaptive_law.tex`                                                                                                          |
| §7, 式(61)–(79)                                            | `resources_07_00073_pump_scheduling_simulation.py` (`PumpParams`, `derive_plant`, `solve_operating_point`, `simulate_cs_control`, `energy_kwh`, `reference_energy_kwh`) |
| §8, 式(88)                                                                                                                          | `realtime_pump_scheduling_simulation.py` (`compute_runoff_Q`, `compute_runoff_Q_with_pump_feedback`, `resample_to_1s`, `cs_control_step`, `run_realtime_stepwise`)      |
| §2.1, 式(36)–(41) | `replicator_sinc_simulation_delay_embedding.py` (`sinc_kernel_matrix`, `A_matrix`, `replicator_rhs`)                                                                    |
| §2.2, 式(42)–(49)           | `rain_runoff_delay_replicator.py` (`build_delay_embedding_rhs`, `estimate_peak_delay`)                                                                                  |
| 式(50)                                                                                                                                                                                                               | 同上, `estimate_peak_delay` 戻り値 `tau_suggest_min`                                                                                                                    |
...
5.  星 清, 山岡 勲,「雨水流法と貯留関数法の相互関係」, 第26回水理講演会論文集, pp.273–278, 1982; 星 清, 村上泰啓,「小流域における総合貯留関数法の開発」, 第31回水理講演会論文集, pp.107–112, 1987 (いずれも「＜参考資料３＞貯留関数法とその適用法」(kohyo-21-k133-1-3), pp.33–35 に引用の式(21)–(24)を経由して参照; §1.2の星・山岡モデルによる分布遅延型拡張の基礎)
